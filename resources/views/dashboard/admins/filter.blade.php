...
<x-filteration :modelName="$modelName">
        ...
        <div class="row">
            ...
            <div class="col-md-3">
                <label class="label-filter">{{ __('lang.branch') }}</label>
                <select class="form-control select2" name="branch" style="width: 100%">
                    <option value="" selected>{{ __('lang.select_branch') }}</option>
                    @foreach($branches as $item)
                        <option value="{{ $item->id }}" @if (isset($data['branch']) && $data['branch'] == $item->id) selected @endif>{{ $item->name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="col-md-2">
                ...
                <select class="form-control select2" name="status" style="width: 100%">
                    ...
                    @foreach(App\Enums\AdminStatuses::cases() as $item)
